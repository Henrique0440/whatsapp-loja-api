...
export default async function handler(req, res) {
  ...
  try {
    ...
    let { nomeLoja, iaActive } = req.body;

    if (typeof nomeLoja !== "string") nomeLoja = false;
    if (nomeLoja && nomeLoja.length > 30) nomeLoja = nomeLoja.slice(0, 30);
    ...
    await lojas.updateOne(
      ...
      { $set: { nomeLoja, iaActive } }
    );

    return res.status(200).json({ sucesso: true, loja: { nomeLoja, iaActive } });
